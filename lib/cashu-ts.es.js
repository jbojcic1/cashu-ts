...
const Rt = T("536563703235366b315f48617368546f43757276655f43617368755f");
function H(n) {
  const t = O(U.concat([Rt, n])), e = new Uint32Array(1), s = 2 ** 16;
  for (let r = 0; r < s; r++) {
...
class _e {
  ...
  connect() {
    return console.log("WSConnection.connect() called", { ws: this.ws, url: this.url, connectionPromise: this.connectionPromise }), this.connectionPromise || (this.connectionPromise = new Promise((t, e) => {
      try {
        this.ws = new this._WS(this.url.toString()), console.log("WSConnection.connect() created ws", { ws: this.ws, url: this.url });
      } catch (s) {
        e(s);
        return;
      }
      this.ws.onopen = () => {
        ...
        console.log("WSConnection.connect() onclose", { ws: this.ws, url: this.url }), this.connectionPromise = void 0;
      };
...
const Ue = "m/129372'/0'", De = (n, t, e) => Nt(
  ...
  const r = Ot.fromMasterSeed(n), o = Ct(t), c = `${Ue}/${o}'/${e}'/${s}`, i = r.derive(c);
...
class K {
  ...
  static createSingleDeterministicData(t, e, s, r) {
    const o = De(e, r, s), c = N(o), i = new TextEncoder().encode(c), a = ue(qe(e, r, s)), { r: u, B_: d } = V(i, a);
    return new K(
...
class Xe {
  ...
  async send(t, e, s) {
    ...
    if (!o && (q(l) != t + w || // if the exact amount cannot be selected
    u || d || f || a || m)) {
  ...
  async swap(t, e, s) {
    ...
    const P = f?.keep || this._keepFactory, D = f?.send, R = this.createSwapPayload(
      ...
      { keep: P, send: D },
      m
    ), { signatures: xt } = await this.mint.swap(R.payload), ht = R.outputData.map((b, v) => b.toProof(xt[v], h)), dt = [], lt = [], ft = Array(R.keepVector.length), pt = Array(ht.length);
  ...
  async onMintQuoteUpdates(t, e, s) {
    ...
    return console.log("CashuWallet.onMintQuoteUpdates() created subscription", { quoteIds: t, subId: r }), () => {
      this.mint.webSocketConnection?.cancelSubscription(r, e), console.log("CashuWallet.onMintQuoteUpdates() cancelled subscription", { quoteIds: t, subId: r });
    };
  }
  /**
  ...
  createOutputData(t, e, s, r, o, c, i) {
    ...
    } else c ? a = K.createP2PKData(c, t, e, o) : i ? a = A(t, e.keys).map((d) => i(d, e)) : a = K.createRandomData(t, e, o);
    return a;
  }
  /**
