...
import { bytesToHex as I, hexToBytes as T } from "@noble/curves/abstract/utils";
import { Buffer as U } from "buffer";
import { hexToBytes as X, bytesToHex as N, randomBytes as mt } from "@noble/hashes/utils";
...
function Ft(n) {
  return U.from(n, "base64");
}
const Ct = T("536563703235366b315f48617368546f43757276655f43617368755f");
function H(n) {
  const t = O(U.concat([Ct, n])), e = new Uint32Array(1), s = 2 ** 16;
  for (let r = 0; r < s; r++) {
    const o = new Uint8Array(e.buffer), c = O(U.concat([t, o]));
    try {
      return B(I(U.concat([new Uint8Array([2]), c])));
    } catch {
      e[0]++;
    }
  }
...
const Qt = (n) => {
  let t;
  return /^[a-fA-F0-9]+$/.test(n) ? t = bt(n) % BigInt(2 ** 31 - 1) : t = R(Ft(n)) % BigInt(2 ** 31 - 1), t;
};
function Wt(n, t) {
...
const Lt = (n, t, e, s) => {
  const r = W.ProjectivePoint.fromPrivateKey(I(n.s)), o = s.multiply(R(n.e)), c = t.multiply(R(n.s)), i = e.multiply(R(n.e)), a = r.subtract(o), u = c.subtract(i), l = Rt([a, u, s, e]);
  return Wt(l, n.e);
}, jt = (n, t, e, s) => {
...
function $t(n) {
  return U.from(n).toString("base64").replace(/\+/g, "-").replace(/\//g, "_").replace(/=+$/, "");
}
function Et(n) {
  return U.from(n, "base64");
}
function St(n) {
  ...
  return Gt(U.from(t).toString("base64"));
}
function Ht(n) {
  const t = U.from(zt(n), "base64").toString();
  return JSON.parse(t);
}
function zt(n) {
...
class it {
  ...
  toEncodedRequest() {
    ...
    return "creqA" + U.from(e).toString("base64");
  }
  getTransport(t) {
...
function yt(n, t, e, s) {
  ...
  vt(e, "asc").forEach((u) => {
    const l = o.filter((m) => m === u).length, f = Math.max(s - l, 0);
    for (let m = 0; m < f && !(r.reduce((h, d) => h + d, 0) + u > t); ++m)
      r.push(u);
  });
  const i = t - r.reduce((u, l) => u + l, 0);
  return i && A(i, e).forEach((l) => {
    r.push(l);
  }), r.sort((u, l) => u - l);
}
...
function de(n) {
  if (n.proofs.forEach((i) => {
    ...
    throw new Error("can not encode to v4 token if proofs contain non-hex keyset id");
  const e = Kt(n), s = st(e), r = "cashu", o = "B", c = $t(s);
  return r + o + c;
}
function Kt(n) {
  const t = {}, e = n.mint;
...
function $e(n) {
  const t = Object.entries(n).sort((r, o) => +r[0] - +o[0]).map(([, r]) => T(r)).reduce((r, o) => me(r, o), new Uint8Array()), e = O(t);
...
function ze(n) {
  const t = new TextEncoder(), e = Kt(n), s = st(e), r = t.encode("craw"), o = t.encode("B");
  return we(r, o, s);
}
function Ge(n) {
...
class _e {
  constructor(t) {
    this.subListeners = {}, this.rpcListeners = {}, this.rpcId = 0, this.onCloseCallbacks = [], this._WS = ke(), this.url = new URL(t), this.messageQueue = new ge();
  }
  connect() {
    return console.log("WSConnection.connect() called", { ws: this.ws, url: this.url, connectionPromise: this.connectionPromise }), this.connectionPromise || (this.connectionPromise = new Promise((t, e) => {
      try {
        this.ws = new this._WS(this.url.toString()), this.onCloseCallbacks = [], console.log("WSConnection.connect() created ws", { ws: this.ws, url: this.url });
      } catch (s) {
        e(s);
        return;
      }
      this.ws.onopen = () => {
        console.log("WSConnection.connect() onopen", { ws: this.ws, url: this.url }), t();
      }, this.ws.onerror = () => {
        ...
        console.log("WSConnection.connect() onclose", { ws: this.ws, url: this.url }), this.connectionPromise = void 0, this.onCloseCallbacks.forEach((s) => s());
      };
  ...
  async ensureConnection() {
    const t = {
      ...
      2: "CLOSING",
      3: "CLOSED"
    }, e = this.ws ? t[this.ws.readyState] : null;
    console.log("WSConnection.ensureConnection() called", { ws: this.ws, url: this.url, readyState: e }), this.ws?.readyState !== 1 && await this.connect();
  }
  handleNextMesage() {
...
function G(n) {
  ...
  ), typeof n.paid == "boolean" && (n.state = n.paid ? L.PAID : L.UNPAID)), n;
}
function kt(n) {
  return n.state || (console.warn(
...
class S {
  ...
  static async swap(t, e, s, r) {
    ...
    if (!M(i) || !Array.isArray(i?.signatures))
  ...
  static async createMeltQuote(t, e, s, r) {
    ...
    if (!M(a) || typeof a?.amount != "number" || typeof a?.fee_reserve != "number" || typeof a?.quote != "string")
      throw new Error("bad response");
    return a;
  }
  /**
  ...
  static async checkMeltQuote(t, e, s, r) {
    ...
    if (!M(a) || typeof a?.amount != "number" || typeof a?.fee_reserve != "number" || typeof a?.quote != "string" || typeof a?.state != "string" || !Object.values(L).includes(a.state))
      throw new Error("bad response");
    return a;
  }
  /**
  ...
  static async melt(t, e, s, r) {
    ...
    if (!M(a) || typeof a?.state != "string" || !Object.values(L).includes(a.state))
      throw new Error("bad response");
    return a;
  }
  /**
  ...
  static async restore(t, e, s) {
    ...
    if (!M(o) || !Array.isArray(o?.outputs) || !Array.isArray(o?.signatures))
      throw new Error("bad response");
    return o;
  }
  async restore(t) {
  ...
  async connectWebSocket() {
    if (console.log("CashuMint.connectWebSocket() called", { ws: this.ws, mintUrl: this._mintUrl }), this.ws)
...
function V(n, t, e) {
  const s = H(n);
  t || (t = R(W.utils.randomPrivateKey()));
...
function Ke(n, t, e) {
  const s = Te(t, e), r = X(n), o = nt.sign(s, r);
  return N(o);
}
class J {
  constructor(t, e, s) {
...
const Ue = "m/129372'/0'", De = (n, t, e) => Nt(
  n,
  t,
  e,
  0
  ...
  const r = Ot.fromMasterSeed(n), o = Qt(t), c = `${Ue}/${o}'/${e}'/${s}`, i = r.derive(c);
...
class K {
  ...
  toProof(t, e) {
    ...
    const r = {
      ...
      C_: B(t.C_),
      dleq: s
    }, o = B(e.keys[t.amount]), c = Me(r, this.blindingFactor, this.secret, o);
    return {
      ...et(c),
      ...s && {
        dleq: {
  ...
  static createSingleRandomData(t, e) {
    const s = N(mt(32)), r = new TextEncoder().encode(s), { r: o, B_: c } = V(r);
    return new K(
  ...
  static createSingleDeterministicData(t, e, s, r) {
    const o = De(e, r, s), c = N(o), i = new TextEncoder().encode(c), a = ue(qe(e, r, s)), { r: u, B_: l } = V(i, a);
    return new K(
      new J(t, l, r).getSerializedBlindedMessage(),
      u,
      i
    );
  }
...
class Xe {
   ...
   */
  constructor(t, e) {
    this._keys = /* @__PURE__ */ new Map(), this._keysets = [], this._seed = void 0, this._unit = Ne, this._mintInfo = void 0, this._denominationTarget = Be, this.mint = t;
    let s = [];
   ...
   */
  async receive(t, e) {
    const { requireDleq: s, keysetId: r, outputAmounts: o, counter: c, pubkey: i, privkey: a, outputData: u, p2pk: l } = e || {};
    ...
    const m = q(t.proofs) - this.getFeesForProofs(t.proofs);
    let h;
    ...
    const d = this.createSwapPayload(
      ...
      h,
      l
    ), { signatures: w } = await this.mint.swap(d.payload), y = d.outputData.map((k, p) => k.toProof(w[p], f)), g = [];
    return d.sortedIndices.forEach((k, p) => {
      g[k] = y[p];
    }), g;
  }
  /**
  ...
  async send(t, e, s) {
    ...
    if (i && (e = e.filter((y) => y.dleq != null)), q(e) < t)
      throw new Error("Not enough funds available to send");
    const { keep: h, send: d } = this.selectProofsToSend(
      e,
      t,
      s?.includeFees
    ), w = c ? this.getFeesForProofs(d) : 0;
    if (!o && (q(d) != t + w || // if the exact amount cannot be selected
    u || l || f || a || m)) {
    ...
    if (q(d) < t + w)
      throw new Error("Not enough funds available to send");
    return i ? { keep: h, send: d } : { keep: h, send: $(d) };
  }
  selectProofsToSend(t, e, s) {
    const r = t.sort((h, d) => h.amount - d.amount), o = r.filter((h) => h.amount <= e).sort((h, d) => d.amount - h.amount), i = r.filter((h) => h.amount > e).sort((h, d) => h.amount - d.amount)[0];
    if (!o.length && i)
      return {
    ...
    if (!o.length && !i)
      return { keep: t, send: [] };
    let a = e, u = [o[0]];
    const l = [], f = s ? this.getFeesForProofs(u) : 0;
  ...
  async swap(t, e, s) {
    ...
    const { includeFees: o, keysetId: c, counter: i, pubkey: a, privkey: u, proofsWeHave: l, outputData: f, p2pk: m } = s || {}, h = await this.getKeys(c), d = e;
    let w = t;
    const y = q(e);
    ...
    const P = f?.keep || this._keepFactory, D = f?.send, C = this.createSwapPayload(
      w,
      d,
      h,
      r,
      ...
      { keep: P, send: D },
      m
    ), { signatures: xt } = await this.mint.swap(C.payload), ht = C.outputData.map((b, v) => b.toProof(xt[v], h)), lt = [], dt = [], ft = Array(C.keepVector.length), pt = Array(ht.length);
    return C.sortedIndices.forEach((b, v) => {
      ft[b] = C.keepVector[v], pt[b] = ht[v];
    }), pt.forEach((b, v) => {
      ft[v] ? lt.push(b) : dt.push(b);
    }), {
      keep: lt,
      send: dt
    };
  ...
  async restore(t, e, s) {
    ...
    a.forEach((h, d) => l[h.B_] = u[d]);
    const f = [];
    let m;
    for (let h = 0; h < i.length; h++) {
      const d = l[i[h].blindedMessage.B_];
      d && (m = t + h, i[h].blindedMessage.amount = d.amount, f.push(i[h].toProof(d, o)));
    }
    return {
      proofs: f,
  ...
  async mintProofs(t, e, s) {
    let { outputAmounts: r } = s || {};
    const { counter: o, pubkey: c, p2pk: i, keysetId: a, proofsWeHave: u, outputData: l, privateKey: f } = s || {}, m = await this.getKeys(a);
    !r && u && (r = {
  ...
  async meltProofs(t, e, s) {
    ...
    return {
      ...
      change: l.change?.map((f, m) => a[m].toProof(f, i)) ?? []
    };
  }
  /**
  ...
  createSwapPayload(t, e, s, r, o, c, i, a, u) {
    const l = e.reduce((p, P) => p + P.amount, 0);
    ...
    const d = [...m, ...h], w = d.map((p, P) => P).sort(
      (p, P) => d[p].blindedMessage.amount - d[P].blindedMessage.amount
    ), y = [
      ...Array(m.length).fill(!0),
      ...Array(h.length).fill(!1)
    ], g = w.map((p) => d[p]), k = w.map((p) => y[p]);
    return {
      payload: {
        inputs: e,
  ...
  async checkProofsStates(t) {
    ...
    for (let c = 0; c < s.length; c += r) {
      ...
      for (let l = 0; l < i.length; l++) {
        const f = u[i[l]];
        if (!f)
          throw new Error("Could not find state for proof with Y: " + i[l]);
        o.push(f);
      }
    }
    return o;
   ...
   * @returns
   */
  async onMintQuoteUpdates(t, e, s, r = () => {
  }) {
    if (console.log("CashuWallet.onMintQuoteUpdates() called", { quoteIds: t }), await this.mint.connectWebSocket(), !this.mint.webSocketConnection)
    ...
    return this.mint.webSocketConnection.onClose(r), console.log("CashuWallet.onMintQuoteUpdates() created subscription", { quoteIds: t, subId: o }), () => {
      this.mint.webSocketConnection?.cancelSubscription(o, e), console.log("CashuWallet.onMintQuoteUpdates() cancelled subscription", { quoteIds: t, subId: o });
    };
  }
  /**
  ...
  createOutputData(t, e, s, r, o, c, i) {
    ...
    } else c ? a = K.createP2PKData(c, t, e, o) : i ? a = A(t, e.keys).map((l) => i(l, e)) : a = K.createRandomData(t, e, o);
    return a;
  }
  /**
...
class xe {
   ...
   */
  async mintProofs(t, e, s) {
    const r = await this.getKeys(s?.keysetId), o = K.createRandomData(t, r), c = {
      outputs: o.map((u) => u.blindedMessage)
    }, { signatures: i } = await this.mint.mint(c, e), a = o.map((u, l) => u.toProof(i[l], r));
    if (a.some((u) => !qt(u, r)))
...
async function Ze(n, t, e) {
  const s = new Q(t);
  return (await new xe(s).mintProofs(n, e)).map((c) => Oe(c));
}
export {
  Q as CashuAuthMint,
  xe as CashuAuthWallet,
  S as CashuMint,
  Xe as CashuWallet,
